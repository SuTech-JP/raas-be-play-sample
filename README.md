...
本サンプルは `raas-client-java` を GitHub Packages から解決する。  
事前に以下のファイルを作成すること。

**`~/.sbt/1.0/credentials.properties`**

```properties
realm=GitHub Package Registry
host=maven.pkg.github.com
user=GitHubユーザー名
password=GitHubトークン（read:packages スコープ必要）
```

GitHub トークンは [Settings → Developer settings → Personal access tokens](https://github.com/settings/tokens) で発行する。
